...
-- ─── Bull spawn (client-side, mirrors what server would do) ───────────────────

local function spawnBull()
	local bullTrove = trove:Extend()

	local visual = bullTrove:Add(getVisual())
	visual.CFrame  = CFrame.new(0, 0, 0)
	visual.Parent  = workspace

	CollectionService:AddTag(visual, "SpainBull")
	...
	local function resolveTrack()
		local state = visual:GetAttribute("BullState")
		...
		if state == "Walk" and visual:GetAttribute("Walking") == true then return tracks.Walk end
		return tracks.Idle
	end
	...
	updateTrack()
	bullTrove:Add(visual:GetAttributeChangedSignal("BullState"):Connect(updateTrack))
	bullTrove:Add(visual:GetAttributeChangedSignal("Walking"):Connect(updateTrack))

	return bullTrove
end

-- ─── Trait loop (client mirrors server logic) ──────────────────────────────────
...
-- ─── Spawn the bull immediately on event start ─────────────────────────────────
