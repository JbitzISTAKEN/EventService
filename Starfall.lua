...
repeat task.wait() until EventController:GetActiveEventData("Starfall")
...
local function loop()
    while EventController:GetActiveEventData("Starfall") do
        local waitTime = math.random(25) / 100
        task.wait(waitTime)

        if not EventController:GetActiveEventData("Starfall") then break end
