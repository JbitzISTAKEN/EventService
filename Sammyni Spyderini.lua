...
local MIN_WANDERS              = 3
local MAX_WANDERS              = 5
local PLAYER_ATTACK_CHANCE     = 15
local PLAYER_ATTACK_DISTANCE   = 5
local PLAYER_CHASE_MAX_TIME    = 12
...
local function getRandomPositionNearTarget(targetPos)
	local parts = getWanderParts()
	for _ = 1, 20 do
...
local function doAttackAnimal(spider)
	local model  = spider.Model
...
local function doAttackPlayer(spider)
	local model  = spider.Model
	local target = spider.Target
	if not model or not target then return end
	...
	local char = target.Character
	if not reached or not isActive or not char or not char.PrimaryPart then
		if isActive then retireSpider(spider) end
		return
	end

	local myPos    = model.PrimaryPart.Position
	local toTarget = char.PrimaryPart.Position - myPos
	local flat     = Vector3.new(toTarget.X, 0, toTarget.Z)
	if flat.Magnitude > 0.1 then
		model:PivotTo(CFrame.lookAt(myPos, myPos + flat.Unit))
	end
	...
	if isActive and char and char.PrimaryPart then
		ClientEventUtils.playBurst(burstAsset, char.PrimaryPart, {
			ReplicatedStorage.Sounds.Events["Sammyni Spyderini"].Hit,
		})
	end
...
local function spawnAttackSpider(target, isPlayer)
	...
	local spawnPos = getRandomPositionNearTarget(targetPos)
	...
	spiderTrove:Add(function()
		if not isPlayer and target and target.Parent then
	...
	spiderTrove:Add(task.spawn(function()
		...
		spider.State = "Idle"
		if spider.IsPlayerTarget then
			startBehavior(spider, "Attack", doAttackPlayer)
		else
			startBehavior(spider, "Attack", doAttackAnimal)
		end
	end))
...
local function main()
	...
	eventTrove:Add(task.spawn(function()
		while isActive do
			...
			if math.random(1, 100) <= PLAYER_ATTACK_CHANCE then
				...
				if #validPlayers > 0 then
					...
					spawnAttackSpider(selected, true)
					continue
				end
			end
			...
			if #candidates > 0 then
				...
				spawnAttackSpider(selected, false)
			end
		end
	end))
