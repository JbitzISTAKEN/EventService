...
local function spawnBike(startCF, endCF, jumpCF)
    ...
    local duration = math.abs(endCF.Z - startCF.Z) / BIKE_SPEED
    local elapsed  = 0
    local conn
...
-- ── standing chicletera attached to server anchor part ────────────────────
local function attachStanding(anchorPart)
    local addObj = Trove.new()
    local pivot  = anchorPart:GetPivot() * CFrame.fromOrientation(0, math.pi, 0)
    ...
    -- server sets ForceSpray on the anchor part — watch that, not the clone
    addObj:Add(anchorPart:GetAttributeChangedSignal("ForceSpray"):Connect(function()
        if anchorPart:GetAttribute("ForceSpray") then
            task.spawn(function()
    ...
    -- face the spray target
    addObj:Add(task.spawn(function()
        while anchorPart.Parent and ReplicatedStorage:GetAttribute("ChicleteiraBicicleteiraEvent") do
            task.wait(0.1)
            local target    = anchorPart:GetAttribute("ForceSpray")
    ...
    addObj:Add(anchorPart.AncestryChanged:Connect(function()
...
-- ── watch for the Chicleteras folder the server drops ─────────────────────
local function watchChicleteras()
    local clones = workspace:WaitForChild("Chicleteras", 60)
    if not clones then
        warn("[Chicleteira] Chicleteras folder never appeared")
        return
    end

    for _, part in clones:GetChildren() do
        task.spawn(attachStanding, part)
    end

    managedObj:Add(clones.ChildAdded:Connect(function(part)
        task.spawn(attachStanding, part)
    end))
end

-- ── fire bikes then watch for standing models ─────────────────────────────
